...
/**
 * Toggles use existing only on a template vehicle.
 * @param tile unused
 * @param flags type of operation
 * @param p1 the template vehicle's index
 * @param p2 unused
 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdToggleTemplateExistingOnly(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	// Identify template to toggle
	TemplateVehicle *template_vehicle = TemplateVehicle::GetIfValid(p1);
	...
	if (should_execute) {
		template_vehicle->ToggleUseExistingOnly();
